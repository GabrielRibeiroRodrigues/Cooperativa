...
def criar_tipo_servico(request):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem cadastrar tipos de serviço.')
...
def editar_tipo_servico(request, tipo_id):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem editar tipos de serviço.')
        return redirect('lista_tipos_servico')
...
def excluir_tipo_servico(request, tipo_id):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem excluir tipos de serviço.')
        return redirect('lista_tipos_servico')
...
def lista_trabalhadores(request):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem buscar trabalhadores.')
        return redirect('home')
    ...
    ofertas = TrabalhadorServico.objects.select_related('trabalhador', 'tipo_servico').filter(
        ...
        trabalhador__is_active=True,
    )

    if query:
        ofertas = ofertas.filter(
            Q(trabalhador__first_name__icontains=query)
            | Q(trabalhador__last_name__icontains=query)
            | Q(trabalhador__username__icontains=query)
            | Q(tipo_servico__nome__icontains=query)
            | Q(descricao_experiencia__icontains=query)
        )

    if tipo_id:
        ofertas = ofertas.filter(tipo_servico_id=tipo_id)

    if localizacao:
        ofertas = ofertas.filter(localizacao__icontains=localizacao)

    if disponivel_agora == '1':
        ofertas = ofertas.filter(disponivel_agora=True)
    ...
    context = {
        ...
        'filtros': {
            ...
            'disponivel_agora': disponivel_agora,
        },
    }
...
def detalhe_trabalhador(request, trabalhador_id):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem acessar este perfil.')
        return redirect('home')
    ...
    servico_chat = (
        Servico.objects.filter(contratante=request.user, trabalhador=trabalhador)
...
def meus_servicos(request):
    if request.user.role != 'trabalhador':
        messages.error(request, 'Apenas trabalhadores podem gerenciar seus serviços.')
        return redirect('home')
...
def editar_meu_servico(request, servico_id):
    ...
    return render(
        request,
        'core/meus_servicos.html',
        {
            'form': form,
            'edicao': servico_oferecido,
            'lista': request.user.servicos_oferecidos.select_related('tipo_servico').all(),
        },
    )
...
def toggle_disponivel_agora(request, servico_id):
    ...
    return JsonResponse(
        {
            'ok': True,
            'disponivel_agora': servico_oferecido.disponivel_agora,
...
def publicar_demanda(request):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem publicar demandas.')
        return redirect('home')
...
def lista_demandas(request):
    if request.user.role != 'trabalhador':
        messages.error(request, 'Apenas trabalhadores podem visualizar demandas abertas.')
        return redirect('home')
    ...
    if somente_compativeis == '1':
        demandas = demandas.filter(tipo_servico_id__in=tipo_ids_trabalhador)

    if q:
        demandas = demandas.filter(
            Q(titulo__icontains=q)
            | Q(descricao__icontains=q)
            | Q(tipo_servico__nome__icontains=q)
            | Q(contratante__first_name__icontains=q)
            | Q(contratante__last_name__icontains=q)
        )

    if tipo_servico:
        demandas = demandas.filter(tipo_servico_id=tipo_servico)

    if localizacao:
        demandas = demandas.filter(localizacao__icontains=localizacao)

    demandas = [d for d in demandas.order_by('data_servico', '-data_criacao') if d.vagas_disponiveis > 0]

    inscricoes_ids = set(
        request.user.inscricoes_demandas.values_list('demanda_id', flat=True)
    )
    ...
    context = {
        ...
        'tipos_servico': request.user.servicos_oferecidos.select_related('tipo_servico').values_list(
            'tipo_servico__id', 'tipo_servico__nome'
        ).distinct(),
        'filtros': {
            'q': q,
            'tipo_servico': tipo_servico,
            'localizacao': localizacao,
            'somente_compativeis': somente_compativeis,
        },
    }
...
def detalhe_demanda(request, demanda_id):
    ...
    if request.user.role == 'trabalhador':
        inscricao_usuario = InscricaoDemanda.objects.filter(demanda=demanda, trabalhador=request.user).first()
        tem_tipo_compativel = request.user.servicos_oferecidos.filter(tipo_servico=demanda.tipo_servico).exists()
        ...
        if request.method == 'POST':
            ...
            form = InscricaoDemandaForm()
    else:
        form = None

    inscricoes = None
    if request.user.role == 'contratante' and demanda.contratante_id == request.user.id:
...
def atualizar_inscricao(request, inscricao_id, acao):
    ...
    inscricao = get_object_or_404(
        InscricaoDemanda.objects.select_related('demanda', 'trabalhador'),
        id=inscricao_id,
        demanda__contratante=request.user,
    )
...
def minhas_demandas(request):
    if request.user.role != 'contratante':
        messages.error(request, 'Apenas contratantes podem acessar esta área.')
        return redirect('home')
...
def minhas_inscricoes(request):
    if request.user.role != 'trabalhador':
        messages.error(request, 'Apenas trabalhadores podem acessar esta área.')
        return redirect('home')
